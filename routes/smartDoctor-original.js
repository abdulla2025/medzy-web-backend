...
async function initializeDependencies() {
  if (dependencyCache.initialized) return;
  
  try {
    const tesseractModule = await import('tesseract.js');
    dependencyCache.tesseract = tesseractModule.default;
    console.log('✅ Tesseract.js loaded successfully');
  } catch (error) {
    console.warn('⚠️ Tesseract.js not available, OCR features disabled');
  }
  
  try {
    const prescriptionAI = await import('./prescriptionAI.js');
    dependencyCache.prescriptionAI = prescriptionAI.parsePrescriptionText;
    console.log('✅ Prescription AI loaded successfully');
  } catch (error) {
    console.warn('⚠️ Prescription AI not available, AI parsing disabled');
  }
  
  dependencyCache.initialized = true;
}
...
router.post('/extract-prescription', authenticateToken, (req, res) => {
  ...
  upload.single('prescriptionImage')(req, res, async (err) => {
    ...
    try {
      ...
      if (!dependencyCache.tesseract) {
        return res.status(503).json({ 
          error: 'OCR service not available in production environment',
          extractedData: {
            medicines: [],
            instructions: "OCR processing not available. Please enter prescription details manually.",
      ...
      const { data: { text } } = await dependencyCache.tesseract.recognize(imagePath, 'eng', {
        logger: m => {
      ...
      let extractedData;
      if (dependencyCache.prescriptionAI) {
        extractedData = await dependencyCache.prescriptionAI(text);
      } else {
        extractedData = {
          medicines: [],
          instructions: `Extracted text: ${text}\n\nAI parsing not available. Please review manually.`,
          rawText: text
        };
      }
